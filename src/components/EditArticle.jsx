...
function EditArticle() {
  ...
  const fetchArticle = async () => {
    try {
      ...
      // Không cố gắng set data ở đây nữa, chúng ta sẽ xử lý trong onReady
    } catch (err) {
      console.error("Error fetching article:", err);
      // Chỉ đặt lỗi nếu đó không phải là lỗi liên quan đến editor
      if (
        !(
          err instanceof TypeError &&
          err.message.includes("setData is not a function")
        )
      ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Log formData to verify it contains thumbnailUrl
      ...
      let articleData = {
        ...
        thumbnailUrl: formData.thumbnailUrl, // Using thumbnailUrl consistently
      };

      // Log final article data to confirm structure
      console.log("Article data being submitted:", articleData);
      ...
      if (isEditMode) {
        ...
        // Lưu articleId để xử lý trong tương lai nếu cần
        articleId = response.data.articleId;
      }

      // Ghi log articleId để tránh lỗi unused
      console.log("Bài viết đã được lưu với ID:", articleId);
      ...
      // Nếu lỗi 403 (Forbidden), có thể thử refresh token và thử lại
      if (err.response && err.response.status === 403) {
        try {
          await refreshToken();
          // Thử lại nghiệp vụ sau khi đã refresh token
          handleSubmit(e);
          return;
        } catch (refreshError) {
          // Nếu refresh token cũng thất bại, thì đưa người dùng về trang login
